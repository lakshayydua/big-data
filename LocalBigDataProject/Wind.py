...
for criteria_gas in ['44201']:#,'42401','42101','42602']:
   ...
   scheme = types.StructType([types.StructField('Month', types.IntegerType(), True),
                              types.StructField('Year', types.IntegerType(), True),
                              types.StructField('wind_pred_' + criteria_gas, types.IntegerType(), True),
   ...
   training = spark.read.csv("/home/ldua/Desktop/BigDataProject/" + str(criteria_gas) + "/" + str(criteria_gas) + ".csv",
   ...
   for i in stateval:
       ...
       #for j in monthval:
       #print(j['Month'])
       df = training.select('Month','Year','AM_Wind').where((training['State Code'] == i["State Code"])) #& (training['Month'] == j['Month']))
       df.show()
       df_test = testing.select('Month','Year').where(
           (testing['State Code'] == i["State Code"])) #& (testing['Month'] == j['Month']))
       df_test.show()
       prediction_Col_Name = "wind_pred_" + str(criteria_gas)
       #,"Observation Percent", "Observation Count","Arithmetic Mean","AM_Wind", "AM_Wind", "AM_RH", "AM_Press"
...
for i in range(len(code)):
   ...
   if i == 0:
       pred_f = pred
       pred_f = pred_f.select(pred['State Code'], pred['Year'], pred['Month'], pred['wind_pred_' + code[i]])
       continue

   pred = pred.select(pred['State Code'].alias('sc'), pred['Year'].alias('y'), pred['Month'].alias('m'),
                      pred['wind_pred_' + code[i]])
...
pred_f.show()
pred_f.coalesce(1).write.csv('/home/ldua/Desktop/BigDataProject/Output/wind_predictions', header=True)

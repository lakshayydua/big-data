...
for criteria_gas in ['44201']:#,'42401','42101','42602']:
   ...
   scheme = types.StructType([types.StructField('Month', types.IntegerType(), True),
                              types.StructField('Year', types.IntegerType(), True),
                              types.StructField('temp_pred_' + criteria_gas, types.IntegerType(), True),
   ...
   for i in stateval:
       ...
       #for j in monthval:
       #print(j['Month'])
       df = training.select('Month','Year','AM_Temp').where((training['State Code'] == i["State Code"])) #& (training['Month'] == j['Month']))

       df_test = testing.select('Month','Year').where(
           (testing['State Code'] == i["State Code"])) #& (testing['Month'] == j['Month']))

       prediction_Col_Name = "temp_pred_" + str(criteria_gas)
...
for i in range(len(code)):
   ...
   if i == 0:
       pred_f = pred
       pred_f = pred_f.select(pred['State Code'], pred['Year'], pred['Month'], pred['temp_pred_' + code[i]])
       continue

   pred = pred.select(pred['State Code'].alias('sc'), pred['Year'].alias('y'), pred['Month'].alias('m'),
                      pred['temp_pred_' + code[i]])
...
pred_f.show()
#pred_f.coalesce(1).write.csv('/home/ldua/Desktop/BigDataProject/Output/temp_predictions', header=True)

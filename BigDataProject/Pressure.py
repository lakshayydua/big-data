...
def load_table(words):
   ...
   dict_data['predicted_pressure'] = float(words[3])
   return dict_data
...
for criteria_gas in ['44201']:#,'42401','42101','42602']:
    ...
    scheme = types.StructType([types.StructField('month', types.IntegerType(), True),
                               types.StructField('year', types.IntegerType(), True),
                               types.StructField('predicted_pressure', types.IntegerType(), True),
    ...
    for i in stateval:
        ...
        #for j in monthval:
        #print(j['Month'])
        df = training.select('month','year','am_press').where((training['state_code'] == i["state_code"])) #& (training['Month'] == j['Month']))

        df_test = testing.select('month','year').where(
           (testing['state_code'] == i["state_code"])) #& (testing['Month'] == j['Month']))

        prediction_Col_Name = "predicted_pressure"
...
for i in range(len(code)):
   ...
   if i == 0:
       pred_f = pred
       pred_f = pred_f.select(pred['state_code'], pred['year'], pred['month'], pred['predicted_pressure'])
       continue
...
words.saveToCassandra(keyspace, 'predicted_pressure')